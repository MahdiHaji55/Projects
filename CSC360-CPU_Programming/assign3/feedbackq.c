...
void initialize_vars() {
	queue_1 = init_queue(); // Initialization of each queue
	queue_2 = init_queue();
	queue_3 = init_queue();
}
...
void handle_instruction(Instruction_t *instruction, int tick) {
    ...
    if(instruction->burst_time == 0) { 
        ...
        task_table[task_id-1].id = task_id; //setting ID
        ...
        int waiting_time; //calculating wait time
        int turn_around_time;
        turn_around_time = task_table[task_id-1].total_wait_time;
        waiting_time = turn_around_time - task_table[task_id-1].total_execution_time;
...
Task_t *peek_priority_task() {
    ...
    // Check queues from highest to lowest priority for any task
    if (is_empty(queue_1) == 0){
        return &task_table[queue_1->start->id - 1];
    }
        
    else if (is_empty(queue_2) == 0){
        return &task_table[queue_2->start->id - 1];
        
    }
        
    else if (is_empty(queue_3) == 0){
        return &task_table[queue_3->start->id - 1];
        
    }
    
    return NULL; // if no task avaiable
}
...
void boost(int tick) {
    ...
    if (tick % BOOST_INTERVAL != 0) return; // checking tick
    
    while (is_empty(queue_3) != 1){//moving the task between th queues
        int tid = queue_3->start->id; 
        dequeue(queue_3); 
        enqueue(queue_1, &task_table[tid-1]); 
        task_table[tid-1].current_queue = 1;
    }
    
    while (is_empty(queue_2) != 1){
        
        int tid = queue_2->start->id;
        dequeue(queue_2); 
        enqueue(queue_1, &task_table[tid-1]); 
        task_table[tid-1].current_queue = 1; 
    ...
    if (current_task != NULL){ //adjusting the reamaining quantum
 ...
 */
void scheduler() {
    //checking busrt time and task quantums
    if (current_task != NULL && remaining_quantum == 0 && current_task->remaining_burst_time > 0){
    ...
    //checking next priority
    Task_t *priority_task = peek_priority_task();
    
    if (priority_task != NULL && current_task != NULL && priority_task != current_task && priority_task->burst_time > 0 && priority_task->current_queue < current_task->current_queue){
        enqueue(get_queue_by_id(current_task->current_queue), current_task);            
        current_task = NULL;
        
    }
    
    //if priority is found
    if (current_task == NULL && priority_task != NULL && priority_task->burst_time > 0){
...
void update_task_metrics() {
    ...
    //update waiting time for all the tasks
    ...
    if (current_task != NULL){
        ...
        task_table[(current_task->id) - 1].total_execution_time++;
